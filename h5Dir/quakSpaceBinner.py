...
if __name__ == "__main__":#blackbox2-CutFromMap.root
    ...
    while i < len(sys.argv):
        if sys.argv[i] == "-f":
            ...
            if(sys.argv[i+1]):
                numBins = int(str(sys.argv[i+1]))
            i += 2
        elif sys.argv[i] == "-frac":
            if(sys.argv[i+1]):
                consideredFrac = float(str(sys.argv[i+1]))
            i += 2
        else:
            print("Invalid input")
            i += 1
    ...
    df.Filter("mass>%s && mass<%s" % (str(minMForHists), str(maxMForHists)))
    npyAll = df.AsNumpy(columns=["loss2"])
    TotalNum = df.Count().GetValue()
    ...
    args = np.argsort(npyAll["loss2"])
    for b in range(numBins):
        bkgThreshold[b] = round(npyAll["loss2"][args[-1*math.floor(TotalNum*consideredFrac - binFrac*b*TotalNum)]],1)

    print(bkgThreshold)

    for b1 in range(numBins):
        startCut = 0
        df2 = df.Filter("loss2>%s && loss2<%s" % (str(bkgThreshold[b1]), str(bkgThreshold[b1+1])))
        npyAll1 = df2.AsNumpy(columns=["loss1"])
        args1 = np.argsort(npyAll1["loss1"])
        TotalNum2 = df2.Count().GetValue()
        print(TotalNum2)
        for b2 in range(1,numBins):
            print(b2)
            print(math.floor((1./float(numBins))*b2*TotalNum2))
            print(args1[math.floor((1./float(numBins))*b2*TotalNum2)])
            sigThresholds[b1][b2] = round(npyAll1["loss1"][args1[math.floor((1./float(numBins))*b2*TotalNum2)]],1)
    ...
    for b in range(numBins):
        ...
        for i in range(startCut,100):
            ...
            #df_tmp = df.Filter("loss2>%s && mass>%s && mass<%s" % (str(i), str(minMForHists), str(maxMForHists)))
            ##npy = df_tmp.AsNumpy(columns=["mass"])
            ##passNum = len(npy['mass'])
            #passNum = df_tmp.Count().GetValue()
            ...
            if(passNum < minCut - b*binFrac*TotalNum):
                ...
                for j in range(10):
                    ...
                    #df_tmp2 = df.Filter("loss2>%s && mass>%s && mass<%s" % (str(float(i-1)+0.1*j), str(minMForHists), str(maxMForHists)))
                    ##npy2 = df_tmp2.AsNumpy(columns=["mass"])
                    ##passNum2 = len(npy2['mass'])
                    #passNum2 = df_tmp2.Count().GetValue()
    ...
    for b1 in range(numBins):
        ...
        for b2 in range(1,numBins):
            for i in range(startCut,100):
                ...
                #df_tmp = df.Filter("loss2>%s && loss2<%s && loss1>%s && loss1<%s && mass>%s && mass<%s" % (str(bkgThreshold[b1]), str(bkgThreshold[b1+1]), str(sigThresholds[b1][b2-1]), str(i), str(minMForHists), str(maxMForHists)))
                ##npy = df_tmp.AsNumpy(columns=["mass"])
                ##passNum = len(npy['mass'])
                #passNum = df_tmp.Count().GetValue()
                ...
                if(passNum > binFrac2D*TotalNum):
                    ...
                    for j in range(10):
                        ...
                        #df_tmp2 = df.Filter("loss2>%s && loss2<%s && loss1>%s && loss1<%s && mass>%s && mass<%s" % (str(bkgThreshold[b1]), str(bkgThreshold[b1+1]), str(sigThresholds[b1][b2-1]), str(float(i)+0.1*j), str(minMForHists), str(maxMForHists)))
                        ##npy2 = df_tmp2.AsNumpy(columns=["mass"])
                        ##passNum2 = len(npy2['mass'])
                        #passNum2 = df_tmp2.Count().GetValue()
    ...
    for i0 in range(numBins):
        for i1 in range(numBins):
            title = "bkgL_%s_%s_sigL_%s_%s" % (str(int(10*bkgThreshold[i0])), str(int(10*bkgThreshold[i0+1])), str(int(10*sigThresholds[i0][i1])), str(int(10*sigThresholds[i0][i1+1])))
            dc_file.write("bin_"+title+"=datacard_JJ_"+title+".txt ")
    ...
    dc_file = open("significanceCorner_TEMPLATE.sh","w")
    for line in dc_templ_file:
    ...
    dc_templ_file = open("./significance_TEMPLATE.sh")
    dc_file = open("significanceFull_TEMPLATE.sh","w")
    for line in dc_templ_file:
        line=line.replace('LISTOFFILES', cutNameList)
        #line=line.replace('MASSMIN', str(minMscan))
        #line=line.replace('MASSMAX', str(maxMscan))
